				...
				this.openFile = function(id, doSave){
					...
					(function(canPaste){
						edt.on('paste', function(o, e){
							...
							if(!canPaste(e.clipboardData.getData('text/plain'))
								&& !canPaste(e.clipboardData.getData('text/html'), true)){
								e.preventDefault();
						...
						edt.on('drop', function(o, e){
							...
							e.preventDefault();
							return(false); */
							if(!canPaste(e.dataTransfer.getData('text/plain'))
								&& !canPaste(e.clipboardData.getData('text/html'), true)){
								e.preventDefault();
								DS.msg('Вставка запрещена', 'red');
								return(false);
					...
					edt.on("gutterClick", (function(tid){return(function(cm, n, gutter){
			...
			DS.ARM.getTaskFiles(idTask, function(d){
				if(d.success){
                   ...
                   for(var i = 0, l = d.data.length; i < l; ++i){
						...
						DS.ARM.saveTaskFile(idTask, d.data[i].name, d.data[i].file, function(){});editor.updateFilesTree();
					}
				}
			});
