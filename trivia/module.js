DS.ready(function(){
	DS.page.registerModule('task', function(){
		...
		var btnCloseTask;var btnExportTask;
...
DS.page.getTaskField = function(key){
		...
		var showComment = function(name, comment){
			DS.create({
				...
				,items: [
					[
						'title'
						,'Комментарий к заданию '+DS.util.htmlescape(name)
						,'->'
				...
				if(d.success){
					DS.create({
						...
						,items: [
							[
								'title'
								,'Введение к заданию '+DS.util.htmlescape(name)
								,'->'
								,{DStype: 'window-button-close'}
							]
							,'<div style="padding: 10px">'
		...
		var endTask = function(cb, skipSave){
           ...
           btnExportTask = null;DS.page.topMenu.removeButton(btnCloseTask);
			btnCloseTask = null;
		...
		var runTrainer = function(idGroup){
			DS.ARM.getTestsList(idGroup, function(d){
				if(d.success && d.data.length){
					...
					DS.page.pushModule('test', {testId: row.id});
				}
			});
					...
					DS.ARM.getTestsGroupList(function(d){
						if(d.success){
							...
							for(var i = 0, l = d.data.length; i < l; ++i){
								...
								DS.addEvent(div, 'click', (function(idGroup){return(function(){
									runTrainer(idGroup);
								});})(row.id));
				...
				if(task.type == _selectedListType){
					...
					if(task.status == 1 || task.status == 3 || task.status == 5){
                                    ...
                                    let fn = function() {
                                        ...
                                        let loadNext = function() {
                                            if (i < task.tools.length) {
                                                ...
                                                let toolStatus = task.tools[i].status; ++i;
                                                fnInitTaskTool(idTool, toolStatus, loadNext);
                                            } else {
                                                DS.progressWindow();
                                            }
                       ...
                       DS.addEvent(btnExportTask, 'click', function(){
                               ...
                               if(d.success){
                                   ...
                                   download(DS.JSON.encode(export_), DS.page.getTaskField('name') + '_export.json', '', false);
                                        ...
                                        DS.ARM.saveTask(_taskData, function(d){
                                            if(d.success){DS.msg('Сохранено', 'green');}
                                            else {DS.msg('Ошибка сохранения', 'red');}
                                        });
                                    };
						...
						DS.addEvent(btn, 'click', (function(name, idTask){return(async function(e){
                            e.stopPropagation();
                            ...
                            if (response.ok)
                            {
                                ...
                                DS.create({
				                    ...
				                    ,items: [
                                        ...
                                        ,'<div class="ds-window-scrollable" width="100%">'
                                        ,'<table style={border: 1px solid #000} width="99%" align="left">'
                                        ,'<tr>'
                                        ,'<th style="border: 1px solid #000; border-collapse: collapse;">№</th><th style="border: 1px solid #000; border-collapse: collapse;">Name</th><th style="border: 1px solid #000; border-collapse: collapse;">Size</th><th style="border: 1px solid #000; border-collapse: collapse;"></th>'
                                        ,'</tr>'
                       ...
                       DS.addEvent(btn, 'click', (function(idTask){return(function(e){
					...
					if(task.status == 3 || task.status == 5 || (task.status == 4 && task.has_theory_text)){
						...
						if(task.status == 3 || task.status == 5){
							...
							DS.addEvent(btn, 'click', (function(comment, name){return(function(e){
							...
							DS.addEvent(btn, 'click', (function(id, name){return(function(e){
								showTheory(name, id);							
								e.stopPropagation();
							});})(task.id, task.name));
						}
					...
					if(task.status == 1 || task.status == 3 || task.status == 5){
						if(task.isLocked && false){
							...
							DS.addEvent(div, 'click', (function(idTask){return(function(){
								showTask(idTask);
							});})(task.id));
						}
					}
			...
			DS.ARM.getTaskTypes(function(d){
				if(d.success){
					for(var i = 0, l = d.data.length; i < l; ++i){
						...
						DS.addEvent(div, 'click', (function(tab){return(function(){fnActivateTab(tab);});})(d.data[i].value));
...
//##########################################################################
		...
		DS.addEvent(DS, 'darkmode/deactivate', function(){fnModeChange(false);});
		DS.addEvent(DS, 'darkmode/activate', function(){fnModeChange(true);});
